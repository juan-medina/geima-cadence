...
`Control` component nodes cross-faded in place, with cancel backing out one step.
Keeping them as separate components — rather than one monolithic menu — is what
lets the same **settings panel** be dropped into both the Menu and the pause
menu unchanged.

Moving between panels is a crossfade: the current panel's opacity is faded to
zero and it is hidden, the next panel is revealed at zero opacity and faded up,
menu input is disabled for the brief crossover so nothing is clickable mid-swap,
and focus is handed to the first control of the incoming panel so a controller
stays usable without a mouse. Cancel runs the same move in reverse, back one
...
metadata — and the menus are drawn from it. This catalog is design data that
never moves. Splitting song bytes into per-song packs is a wholly separate
concern that exists only because a web build cannot ship every song in a single
download; packs change only *where a selected id's bytes come from*, never the
catalog or the selection flow above it.

**Biome art is not in the song pack.** A biome is shared across all the songs of
its zone, so packing its art per-song would duplicate it. The song pack carries
...
*   A song loaded at runtime must keep **Stream** playback on web, not Sample:
    the sync clock reads the playback position, which Sample playback breaks.
    Streaming from a downloaded file is exactly where this could regress.
*   The beatmap travels with its song (same basename), so wherever a song
    resolves from, its `.json` sits beside it.
*   No counts belong in this section. If the design is right it does not name how
    many biomes or songs exist; those are implementation details that change
